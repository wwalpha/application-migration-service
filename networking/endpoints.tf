...
resource "aws_security_group" "allow_tls" {
  ...
  ingress {
    ...
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = [module.vpc_dst.vpc_cidr_block]
  }
...
# ----------------------------------------------------------------------------------------------
# AWS VPC Endpoint - S3 Gateway
# ----------------------------------------------------------------------------------------------
resource "aws_vpc_endpoint" "s3" {
  vpc_id            = module.vpc_dst.vpc_id
  service_name      = "com.amazonaws.ap-northeast-1.s3"
  vpc_endpoint_type = "Gateway"
  route_table_ids   = module.vpc_dst.private_route_table_ids
  tags = {
    Name = "${var.prefix}_s3"
  }
}
...
resource "aws_vpc_endpoint" "ssm" {
  ...
  subnet_ids          = module.vpc_dst.private_subnets
